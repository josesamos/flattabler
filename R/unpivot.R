...
#'
#' @export
unpivot2 <- function(pt, include_page, na.rm) UseMethod("unpivot2")

#' @rdname unpivot
#' @export
unpivot2.pivot_table <- function(pt,
                                include_page = TRUE,
                                na.rm = TRUE)
{
  n_col <- pt$n_col_labels
  ...
  for (j in 1:n_col) {
    df[, k] <- rep(pt$df[(n_row + 1):nrow(pt$df), j], ncol(pt$df)-n_col)
    k <- k + 1
  }
  for (i in 1:n_row) {
    df[, k] <- rep(t(pt$df[i, (n_col + 1):ncol(pt$df)]), nrow(pt$df)-n_row)
    k <- k + 1
  }
  df[, k] <- as.vector(t(pt$df[(n_row + 1):nrow(pt$df), (n_col + 1):ncol(pt$df)]))
  if (na.rm) {
    df <- df[!is.na(df[, k]),]
  }
  tibble::tibble(df)
